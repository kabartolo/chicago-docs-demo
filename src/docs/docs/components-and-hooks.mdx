...
Chicago Docs comes with several components that help make it quick and easy for your users to find information on your product or API. Features such as site search and sidebar navigation mean your users won't get frustrated trying to find what they need.

<TwoColumnSection>

### Terminology

In this section, a *child* is a block of Markdown text between JSX tags (shortcodes); these blocks of text are passed to the React component as a prop called `children`. There should be a blank line above and below each Markdown child.

A *prop* is a value passed to the React component directly.

In the following example, the text `"Attention!"` is a child of the Alert component, and `variant` is a prop (the string `"warning"` will be passed to the component to determine the background color of the Alert box).
...
```js
---
title: Title of example MDX file
---
...
| `children` | React node or array of nodes | required | | MDX text placed between Alert tags. This text is rendered inside the Alert box. |
...
| `labels` | array of strings | optional | `[]` | Array of strings describing the language or other aspect of each code block (the number of labels should equal the number of code blocks and be in the same order). If more than one label is present, the labels are displayed in a select box allowing users to switch between examples. A single label will be displayed as text. |
| `children` | React node or array of nodes | required | | MDX code blocks placed between CodeExample tags. |
...
This component creates an external link that opens in a new tab, which is indicated by an external-window icon. This component is used automatically in docs for any <a href="/docs/mdx-guide/mdx-guide/#links">Markdown or HTML links</a> with an `href` attribute starting with `'http'`. For example, `<a href="https://en.wikipedia.org">Wikipedia</a>` becomes <a href="https://en.wikipedia.org">Wikipedia</a>. You can also use the component directly if you prefer:
...
| Name | Type | Info | Default | Description |
| ---- | ---- | ---- | ------- | ----------- |
| `href` | string | required | | External URL that will open in a new tab when the component is clicked.|
...
This will render a fluid image (try changing the size of your screen):
...
The following example demonstrates use of the PostList component:
...
This component will print a list of docs in the `tutorials` directory of the `docsPath` directory, such as `src/docs/tutorials/how-to-install-ubuntu.mdx`.
...
| `children` | React node or array of nodes | required | | Markdown table placed between PropertyTable tags. Table headers should be `Name`, `Type`, `Info`, `Default`, and/or `Description` (order not important). Other table headers will be ignored. |
...
*Note.* If a value for the `Default` column is included, it will appear following the string `'default:'`. You can get around this by omitting a default value and adding it elsewhere:
...
| Name | Type | Info | Default | Description |
| ---- | ---- | ---- | ------- | ----------- |
| `type` | string | optional (default: `'array'`) | | Type of the resource, one of `'array'` or `'object'`. |
...
The TOC component is added automatically to MDX files in `src/docs` (or the `docsPath` defined in the <a href="/docs/configuration/site-options/#theme-options">theme options</a>) when the <a href="/docs/configuration/site-options/#theme-options">`alwaysShowTOC`</a> theme option is set to `true`. This can be overriden by the `showTOC` <a href="/docs/configuration/site-options/#frontmatter">frontmatter field</a> for an individual post.

You can also insert a TOC component in any MDX file. It will produce a TOC for the current file.
...
This results in the following component (a table of contents for the current doc):
...
| `title` | string | optional | `''` (empty string) | Title displayed above list of contents. |
...
| Fragment link | Jumps to a different location on the same page without a page refresh | Fragment path (e.g., `#theme-ui-components`) | `<a>` tag | [Theme-UI Components](#theme-ui-components) | 

Create a link using either Markdown or HTML:
...
The programming language (e.g., `js`) should be specified after the first three backticks (```` ``` ````) so that <a href="https://www.prismjs.com">Prism.js</a> can determine how to highlight the code. Since <a href="https://github.com/FormidableLabs/prism-react-renderer">`prism-react-renderer`</a> is used behind the scenes, Chicago Docs supports only those languages supported by that plugin. These languages are listed <a href="https://github.com/FormidableLabs/prism-react-renderer/blob/master/src/vendor/prism/includeLangs.js">here</a>.
...
This adds the line number to the beginning of each line:
...
This produces the following highlights:
...
| Hook | Parameters | Purpose | In Core |
| ---- | ----- | ------- | --- |
| useDocContext | | Fetches the `docId` and sidebar `menu` object for the current doc. | No |
| useDocData | | Fetches all GraphQL data for all docs (see below). | Yes |
...
| useTableOfContents | `{ depth = 6, docId = '', path = '' }` | Fetches the table of contents for all docs in an array of objects, each object including the doc `id`, a `nested` object, and a `flatMap` array. Pass the `docId` or current `path` to get the object for a specific doc. The `depth` value specifies the depth of the `nested` object (does not affect `flatMap`). | Yes |
...
See <a href="/docs/configuration/">Configuration</a> for more details on theme options and frontmatter.
