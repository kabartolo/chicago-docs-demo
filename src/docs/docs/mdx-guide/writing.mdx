...
If you prefer not to use MDX, you can also just write regular Markdown. Elements such as code blocks and tables are automatically styled by the theme (and these styles are <a href="/docs/styling-and-shadowing/#theme-ui">easily customized</a>), so you can focus on writing great content.
...
A doc's frontmatter contains data about it, such as the `title` and `description`. Frontmatter is placed at the top of the MDX file between sets of three hyphens (---). Each frontmatter field should be placed on its own line. 
...
Note that the HTML equivalents can also be used instead of Markdown syntax. For example, you can write `<h1>This is a heading</h1>` instead of `# This is a heading` if you prefer.
