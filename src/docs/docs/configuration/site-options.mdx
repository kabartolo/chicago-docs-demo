...
The `src/pages` directory contains non-docs pages on your site, such as `404.js`. Any MDX files in this folder will be processed by `gatsby-plugin-mdx` but will not use the Doc page template (and will therefore **not** contain breadcrumb links, sidebar navigation, or previous/next navigation).
